...
def load_passages(corpus_path: str) -> pd.DataFrame:
    return pd.read_csv(corpus_path, sep="\t", names=["passage_id", "passage"], dtype={
        "passage_id": int,
        "passage": str
...
def load_queries(queries_path: str) -> pd.DataFrame:
    return pd.read_csv(queries_path, sep="\t", names=["query_id", "query"], dtype={
        "query_id": int,
        "query": str
    }).set_index("query_id")
...
def load_qrels(qrels_path: str) -> pd.DataFrame:
    return pd.read_csv(qrels_path, sep="\t", names=["query_id", "positive_passage_id", "negative_passage_id"], dtype={
        "query_id": int,
...
def load_test_qrels(qrels_path: str) -> pd.DataFrame:
    return pd.read_csv(qrels_path, sep=" ", names=["query_id", "Q0", "passage_id", "rating"], dtype={
        "query_id": int,
        "Q0": str,
        "passage_id": int,
        "rating": int
...
@click.option("--qrels_path", default="qidpidtriples.train.full.2.tsv", type=str)
@click.option("--subsample", default=1_000_000, type=int)
@click.option("--seed", default=42, type=int)
@click.option("--validation_fraction", default=0.1, type=float)
def prepare_data(data_path: str, output_path: str, passages_path: str, queries_path: str, qrels_path: str,
                 subsample: int, seed: int, validation_fraction: float) -> None:
    ...
    train_qrels = qrels_values[:int(len(qrels_values) * (1 - validation_fraction))]
    validation_qrels = qrels_values[int(len(qrels_values) * (1 - validation_fraction)):]
    ...
    write_shards("validation", output_path, passages, queries, validation_qrels)

    print("Writing to test shards")
    test_queries = load_queries(os.path.join(data_path, "msmarco-test2019-queries.tsv"))
...
def write_test_shards(output_path: str, passages: pd.DataFrame, queries: pd.DataFrame, test_qrels: pd.DataFrame):
    test_sink = wds.TarWriter(os.path.join(output_path, f"test.tar"))
    for query_id, _, passage_id, rating in tqdm(test_qrels.values, desc="Writing to test shards"):
...
def write_shards(name: str, output_path: str, passages: pd.DataFrame, queries: pd.DataFrame, qrels):
    ONE_GIGABYTE = 1024 ** 3
    sink = wds.ShardWriter(os.path.join(output_path, f"{name}-%d.tar"), maxsize=ONE_GIGABYTE, maxcount=100000,
                           verbose=0)

    for query_id, positive_passage_id, negative_passage_id in tqdm(qrels, desc="Writing to shards"):
        query = queries.loc[query_id, "query"]
